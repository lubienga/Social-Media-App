...
* 
*/
if(in_array('connect.php', scandir('config/'))) {
	if(file_exists('config/connect.php') && is_file('config/connect.php')) {
		require_once('config/connect.php');
		require_once('model/helper_model.php');
	} else {
...
HTMLDATA;
	...
	public function getHashKey($data) {
		$hashdata_array[] = $data;
		$this->hashdata = $hashdata_array;
		$hashObj = new Helper();
		if(is_a($hashObj, 'Helper')) {
			if(method_exists($hashObj, 'hashKey')) {
				return call_user_method_array('hashKey', $hashObj, $this->hashdata);
			}
		}
	}
	...
	protected function genNewPassword() {
		$keyObj = new Helper();
		if(is_a($keyObj, 'Helper')) {
			if(method_exists($keyObj, 'tokenKey')) {
				return call_user_method('tokenKey', $keyObj);
			}
		}
	}
